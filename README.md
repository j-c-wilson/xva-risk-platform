...
| **DVA** (Debit Valuation Adj.) | Own Default Risk (Bilateral) | $(1 - R_{\text{own}}) \cdot \text{PD}_{\text{own}} \cdot \text{ENE}$ <br> • $R_{\text{own}}$: Own institution recovery rate <br> • $\text{PD}_{\text{own}}$: Own default probability <br> • $\text{ENE}$: Expected Negative Exposure (our debt to counterparty) |
